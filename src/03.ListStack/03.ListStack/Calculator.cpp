...
Calculator::Calculator(bool isListStack) {
	if (isListStack == true) {
		...
		operands = new TListStack<std::string>();
		reverseStackOfOperands = new TListStack<std::string>();
		calc = new TListStack<double>();
	}
	else {
		operators = new TArrayStack<char>(MAX_SIZE);
		operands = new TArrayStack<std::string>(MAX_SIZE);
		reverseStackOfOperands = new TArrayStack<std::string>(MAX_SIZE);
		calc = new TArrayStack<double>(MAX_SIZE);
	}
}
